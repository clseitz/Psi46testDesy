...
bool CUSB::Open( char serialNumber[] )
{
  ...
  if( ftStatus != FT_OK )  {
	...
	continue;
	  ...
	  ( descriptor.idProduct != 0x6001 || descriptor.idProduct != 0x6014 ) )
	continue;
...
bool CUSB::WaitForFilledQueue( int32_t pSize, int32_t pMaxWait )
{
  ...
  while( ( ( waitCounter*10 < pMaxWait) || pMaxWait < 0 ) && bytesWaiting < pSize ) {
    if( (waitCounter+1) % 100 == 0 ) cout << "USB waiting for " << pSize << " data, t = " << waitCounter*10 << " ms, bytes to fetch so far = " << bytesWaiting << endl;
  ...
  if( waitCounter*10 >= pMaxWait ) {
    ...
    if( bytesWaiting > 0 ) cout << "USB buffer filled with " << bytesWaiting << " bytes, check if given expectation of " << pSize << " is correct! " << endl;
    else cout << "USB NO DATA in buffer! " << endl;
    return false;
  }
