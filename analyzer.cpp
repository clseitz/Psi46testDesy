...
int DecodePixel( const vector<uint16_t> &x, int &pos, PixelReadoutData &pix )
{
  PROFILING;
  ...
  if( pos >= int( x.size() ) ) return 2; // wrong header
  ...
  if( pos >= int(x.size() ) || ( x[pos] & 0x8000 ) ) return 3; // incomplete data
...
vector<PixelReadoutData> GetEvent( const vector<uint16_t> &x, int &pos, int &hdr )
{
  PROFILING;
  ...
  if( pos >= int( x.size() ) ) throw int(2); // wrong header
  ...
  if( pos >= int(x.size() ) ) return vpix; // empty data readout

  if( ( x[pos] & 0x8000 ) ) return vpix; // empty event, new event
  ...
  do {
    ...
    if( pos+1 >= int(x.size() ) || ( x[pos] & 0x4000 ) ) throw int(3); // incomplete data
    pos++;
