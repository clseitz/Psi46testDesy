...
\title{\noindent \textbf{psi46test at DESY}}


\author{\noindent Daniel Pitzl, DESY}
...
While you wait, install the usb driver library: \texttt{libftd2xx.so}
from ftdi.com
...
computer try \texttt{. initb.sh} on Linux)
...
Commands are defined in cmd.cpp. To add a command, put the code in
a new block \texttt{CMD\_PROC(cmdname)\{\}} somewhere in the file
and register it in the \texttt{cmd()} section (towards the end) with
a help string: \texttt{CMD\_REG( cmdname, \textquotedbl{}cmdname <argument>
explain what it does\textquotedbl{} )}. No header files are involved.
...
A test (e.g. a DAC scan) is supposed to put back the original state,
unless a DAC is changed on purpose.

Commands and measurements are written to the log file. This was implemented
for offline parsing, processing, and plotting. It is still useful
for reconstructing the conditions under which a particular test in
a session was executed. Measurements are now also written as 1D and
2D histograms into a ROOT file \texttt{Test.root}, for direct plotting
...
\begin{description}
\item [{rpcinfo}] prints the list of functions available in the DTB SW
via RPC.
\item [{upgrade\ dtb\_v2.21.flash}] load new firmware/software into the
FPGA. Wait until the LEDs are off. Exit psi46test. Power cycle the
DTB (unplugging the power cable) to make sure that the new executable
is loaded from EPROM.
\item [{help}] prints the list of commands defined in cmd.cpp
\item [{info}] prints DTB info
\item [{version}] prints DTB hardware, firmware, and software version numbers
\item [{boardid}] prints the production serial number
\item [{welcome}] play the LED startup sequence
\item [{setled\ bits}] play with the four LEDs on the test board (bit
pattern: 0 all off, 15 all on)
\item [{pon}] low voltage on
\item [{getva}] measure the analog supply voltage on the board
\item [{getvd}] measure the digital supply voltage on the board
\item [{va\ mV}] set the analog supply voltage (range 0 to 3000 mV, 1700
mV is fine)
\item [{vd\ mV}] set the digital supply voltage (range 0 to 3000 mV, 2500
mV is fine)
\item [{poff}] low voltage off. Do this before exiting from psi46test.
\item [{quit}] (or exit) closes the DTB connection, the log and ROOT files
and ends psi46test.
\end{description}
If you want to keep them, \texttt{rename} d.log and/or Test.root before
you start again, otherwise they get overwritten.


\subsection{starting up with a ROC}

Connect a ROC (or module) via SCSI cable to the DTB. You may also
want to connect (negative of about -150 V) bias voltage to the red-ringed
lemo connector.

Start again: \texttt{bin/psi46test c405.log}
\begin{description}
\item [{s405}] execute start-up commands for a given chip (\texttt{script/s405.roc})
\item [{getid}] measure digital current, should be around 22-28\,mA per
ROC
\item [{getia}] measure analog current, should be around 25\,mA per ROC.
If it is around 5\,mA the ROC is not properly programmed. Inspect
the settings in the start script. The problem is either here, or the
ROC is dead.
\item [{deser160}] 2D scan of clock phase and 160\,MHz deserializer phase
(for single ROCs), searching for the proper ROC header 7FA (hex).
Sets the new values, if successful.
\end{description}
The following commands elicit a response from the ROC only if it is
properly set up (Vana, WBC, CalDel, VthrComp are the most critical
DACs). See below for more algorithmic procedures.
\begin{description}
\item [{fire\ col\ row\ {[}nTrig{]}}] pulse one pixel. Columns are 0..51,
rows are 0..79
\item [{arm\ col\ row}] enable column, un-mask one pixel and prepare
it for calibrate pulses
\item [{arm\ col:col\ row:row}] enable range of columns, un-mask ranges
of pixels and prepare for calibrate
\item [{single}] single calibrate event display (one cycle of reset-calibrate-trigger-token
or whatever is programmed in the pattern generator)
\item [{cole\ col}] enable one column
\item [{cole\ col:col}] enable range of columns e.g. cole 0:51 for all)
\item [{pixe\ col\ row}] unmask one pixel
\item [{pixe\ col:col\ row:row}] unmask range of pixels (e.g. pixe 0:51
0:79 for the entire ROC)
\item [{cal\ col\ row}] activate column and row for calibrate
\item [{cal\ col:col\ row:row}] activate ranges of columns and rows for
calibrate (cols and rows independently
\item [{cald}] clear calibrate from all pixels
\item [{mask}] all pixels
\item [{cold\ col}] disable column
\item [{pixd\ col\ row}] disable pixel
\end{description}
...
\hline 
DAC & name & comment & DAC & name & comment\tabularnewline
\hline 
\hline 
1 & Vdig &  & 17 & VoffsetRO & \tabularnewline
\hline 
2 & Vana &  & 19 & Vcomp\_ADC & \tabularnewline
\hline 
3 & Vsf &  & 20 & VIref\_ADC & \tabularnewline
\hline 
4 & Vcomp &  & 22 & VIColOr & \tabularnewline
\hline 
7 & VwllPr &  & 25 & Vcal & \tabularnewline
\hline 
9 & VwllSh &  & 26 & CalDel & \tabularnewline
\hline 
10 & VhldDel &  & 253 & CtrlReg & \tabularnewline
\hline 
11 & Vtrim &  & 254 & WBC & \tabularnewline
\hline 
12 & VthrComp &  & 255 & RBreg & \tabularnewline
\hline 
13 & VIBias\_Bus &  &  &  & \tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
\begin{description}
\item [{optia\ target}] set Vana to get the desired target analog current
{[}mA{]}, .e.g. optia 25
\item [{show}] current DAC settings (presumably, from book-keeping in psi46test;
reading back DACs from the ROC is not possible)
\item [{dac\ number\ value}] set a DAC value (number is the DAC address).
Some DACs have shortcuts:
\item [{vana\ value}] set Vana {[}0:255{]} (check analog current: 1 mA
/ 6 DAC units)
\item [{vthr\ value}] set global threshold VtrhComp {[}0:255{]}
\item [{vcal\ value}] set test pulse amplitude {[}0:255{]}
\item [{ctl}] set control register (0 = small Vcal, 4 = large Vcal, 1 =
ROC off)
\item [{caldel\ col\ row}] measure pixel efficiency vs \texttt{CalDel},
set \texttt{CalDel}
\item [{caldelroc}] scan CalDel for the entire ROC (perfect pixels respond
to all triggers, alive pixels have at least 50\% response), sets CalDel
in the plateau region
\item [{thrmap\ guess}] measure pixel threshold map for current settings
(faster if guess is close to truth)
\item [{vthrcompi}]~
\item [{vthrcomp\ target\,{[}guess{]}}] set VthrComp such that the minimum
pixel threshold is at target Vcal units
\item [{trim\ target}] set Vtrim and trim bits such that all pixel thresholds
are as close as possible to target Vcal units
\item [{effmap\ nTrig}] measure efficiency map (PixelAlive) with n triggers
per pixel
\item [{trimbits}] adjust trim bits to recover maximum efficiency
\item [{wtrim\ chip}] write current trim bits to \texttt{trimParameters\_chip.dat}
\item [{phmap\ ntrig}] measure pixel pulse height map
\item [{tune}] set gain and offset such that the pulse heights of all pixels
are in 80\% of the ADC range, for large and small Vcal, with 10\%
margins against overflows and underflows
\item [{wdac\ chip}] write current DAC settings to \texttt{dacParameters\_chip.dat}
\end{description}
...
DAC is not changed. 
\begin{description}
\item [{effdac\ col\ row\ dac}] count trigger responses (efficiency)
for one pixel vs a DAC
\item [{phdac\ col\ row\ dac}] pulse height (ADC) vs DAC for one pixel
\item [{calsdac\ col\ row\ dac}] sensor calibrate pulse height (ADC)
vs DAC at CtrlReg 4 (high range Vcal) for one pixel
\item [{thrdac\ col\ row\ dac}] threshold (in small Vcal units) vs DAC
for one pixel
\item [{dacdac\ col\ row\ dacx\ dacy}] 2D DAC-DAC scan for one pixel,
pulse height and efficiency
\item [{dacscanroc\ dac\ {[}nTrig{]}}] maps of pulse height and efficiency
vs dac for all pixels (dac 25 at ctl 0 gives S-curves, dac 25 at ctl
4 gives gain calibration, dac 26 gives CalDel)
\item [{gaindac}] calibrated pulse height vs Vcal for all pixels, checks
the gain calibration
\end{description}

\subsection{maps}
\begin{description}
\item [{effmap\ nTrig}] pixel efficiency map (PixelAlive)
\item [{thrmap\ guess}] measure pixel threshold map for current settings
(faster if guess is close to truth)
\item [{phmap\ nTrig}] pulse height map (vary vcal and ctl to explore
full range)
\end{description}
...
\begin{description}
\item [{cals\ col\ row}] active one pixel for sensor calibrate (requires
\texttt{cole col }and\texttt{ pixe col row} to see the response with
\texttt{single})
\item [{calsdac\ col\ row\ dac}] sensor calibrate pulse height (ADC)
vs DAC at CtrlReg 4 (high range Vcal) for one pixel
\item [{calsmap\ nTrig}] sensor calibrate pulse height map
\item [{bbtest\ nTrig}] sensor calibrate pulse height map with bump bond
statistics
\item [{dacscanroc\ dac\ -nTrig}] sensor calibrate (selected by negative
nTrig) pulse height and efficiency vs dac for all pixels (\texttt{dac}
12 at \texttt{ctl} 4 gives bump bond test)
\end{description}
...
\begin{description}
\item [{pgloop\ 1000}] gives a rate of 40\,kHz.
\end{description}
...
for psi46digV2.1, WBC = tct - 6 for earlier digital ROCs, WBC = tct
- 5 for analog ROCs, where tct ist the time between calibrate and
trigger in the pattern generator sequence (e.g. WBC 99 for tct 106).

Fine: CalDel shifts the timing of the test pulse, unit: 1\,DAC =
0.4 ns, dynamic range 0..255 = 100\,ns = 4 BC.


\subsection{threshold trimming}

Symbolic equation: pixelThreshold = globalThreshold - \texttt{Vtrim
(15 - trim bits)}, where the pixel threshold is determined from a
Vcal scan with several triggers per point, searching for the point
with 50\% response. In low Vcal range (CtrlReg 0), a Vcal threshold
of 30 DAC units corresponds to about 1500 electrons signal. \texttt{VthrComp}
and \texttt{Vtrim} are global DACs, affecting the entire ROC, while
the four \texttt{trim bits} can be set for each pixel individually.
As the equation shows, the trimming can only lower the threshold from
the value determined by \texttt{VthrComp}. The \texttt{trim bits}
act inverted: 15 means no effect, while 0 gives the maximum threshold
reduction as allowed by \texttt{Vtrim}. Due to transistor variations
from pixel to pixel the untrimmed threshold distribution (Vtrim 0,
trim bits 15) is rather broad, with an RMS of typically 6 Vcal DAC
units and a non-Gaussian distribution that reflects geographical variations
across the ROC. The goal of the trimming procedure is to sharpen the
threshold distribution to about 1 Vcal DAC unit (50 e) and a mean
value a low as possible, but staying clear by at least 6\,$\sigma$
from the noise level. The dynamic range of the threshold DACs is rather
large (except for digV2 ROCs at nominal analog current), so that thresholds
from 1\,ke to 10\,ke can be reached, in 50\,e steps. The trimming
procedure thus starts with selecting a threshold target (e.g. 30 Vcal
DAC units), and adjusting the DACs and bits to reach that. A second
constraint can be derived from the threshold equation: a smaller value
of Vtrim leads to a closer spacing of the trim bit steps and a sharper
...
the ADC range, for large and small Vcal.


\section{offline processing}

The ROOT and log files from some tests are used for further processing
and analysis.


\subsection{gain calibration and S-curves\label{sub:gaincalib}}
...
\begin{description}
\item [{ctl\ 0}] small Vcal
\item [{dacscanroc\ 25\ nTrig}] measure pulse height vs Vcal for each
pixel, nTrig = 10 takes about 90 s, filling a 2D histogram \texttt{PH\_DAC25\_CR0\_map}
\item [{ctl\ 4}] large Vcal
\item [{dacscanroc\ 25\ nTrig}] filling \texttt{TH2D PH\_DAC25\_CR4\_map}
\end{description}
...
$f$ has an inflection point where $f'$ has a maximum and where $f''$has
a zero, namely at $t_{inf}=\left((p_{2}-1)/p_{2}\right)^{1/p_{2}}$.
The maximum gain is then $f'(t_{inf})$. It turns out that $p_{0}$is
always very close to one (like 0.9998). It is thus tempting to reduce
the number of parameters by setting $p_{0}$to one. Furthermore, $p_{1}$turns
out the rather large ($10^{5}$in Vcal DAC units), inviting one more
approximation:
...
\item [{dacscanroc\ 25\ nTrig}] fills TH2D histograms for puls height
(\texttt{PH\_DAC25\_CR0\_map}) and number of reponses (\texttt{N\_DAC25\_CR0\_map}),
...
This will run for some time and produce two output files: minscurves.ps
(containing all the s-curve plots/fits for each pixel) and fit\_results\_scurve.root
...
\texttt{root -l fit\_results\_scurves-c405-trim36.root}
...
\begin{description}
\item [{bbtest\ nTrig}] fast map of all pixels' responses to \texttt{cals}
pulses, done with the largest pulse (\texttt{CtrlReg} 4, \texttt{Vcal}
255) but with fixed threshold settings.
\item [{dacscanroc\ 12\ -nTrig}] scan global threshold \texttt{VthrComp}
