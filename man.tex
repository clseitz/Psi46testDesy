...
\item [\texttt{tune}] set gain and offset such that the pulse heights
of all pixels are in 80\% of the ADC range, for large and small Vcal,
with 10\% margins against overflows and underflows 
...
Most of the commands don't produced any plots so after some of the steps using the \textbf{\texttt{modthrmap}} command.
\begin{description}
...
\item [\texttt{modvthrcomp 42}] pull lowest threshold pixel down to 42 (has to be safely away from trim target), Figure \ref{fig:thrmap} on the left
\item [\texttt{modcaldel 22 33}]
\item [\texttt{modtrim 32}] trim module to a threshold of 32 (Figure \ref{fig:thrmap} on the left), if Vtrim is set to one then repeat modvthrcomp 42
...
\begin{figure}
\noindent \begin{centering}
\includegraphics[scale=0.40]{thresholdmap_aftermodvthrcomp42_caldel.pdf} 
\includegraphics[scale=0.40]{thresholdmap_aftermodtrim32_caldel.pdf} 
\label{fig:thrmap}
\par\end{centering}
