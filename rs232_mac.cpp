...
char comports[31][19] = {
  ...
  "/dev/rfcomm0", "/dev/rfcomm1", "/dev/ircomm0", "/dev/ircomm1","/dev/tty.UC-232AC"
};

//int comport_number = 16;  //not used, defined in iseg.cpp      // "/dev/ttyUSB0"
int comport_number = 30;
...
int RS232_OpenComport( int comport_number, int baudrate )
{
  ...
  //parameters copied from CuteMon
  ...
  rs_error = tcsetattr( Cport[comport_number], TCSANOW, &newtio );
  if( rs_error == -1 ) {
    close( Cport[comport_number] );
